...
def save_and_show():
    ...
    # Initialize drone
    drone = ARDrone()
    drone.navdata_ready.wait() # Wait for navdata
        ...
        if running:     # If the frame was successfully read.
            ...
            if verify_image('image/now.jpg'):       # If the image is not corrupted.
                ...
                for (x, y, w, h) in faces:
                    ...
                    # compare center of image to center of face to determine offset
                    ...
                    # face center
                    cv2.rectangle(img, (faceCenterX - 1, faceCenterY - 1), (faceCenterX + 1, faceCenterY + 1), (255, 255, 255), 2)

                # center
                cv2.rectangle(img, (frameCenterX - allowance, frameCenterY - allowance), (frameCenterX + allowance, frameCenterY + allowance), (255, 255, 255), 1)

                # actually move the drone!
                drone.move(left=pwr_left, right=pwr_right, up=pwr_up, down=pwr_down)
                # time.sleep(1)
                # drone.move(left=0, right=0, up=0, down=0)
